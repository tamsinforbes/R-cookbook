...
## More {ggplot2} charts with DfT theme
...
Note that to draw out a colour for the points, the code specifies which colour in the list of DfT colours the points will be. In this case, the third colour in the list will be the colour of the points.
...
ggplot(data = accident_pf, aes(x = Police_Force, y = n)) +
  ...
  geom_text(aes(label=Police_Force), size=3, hjust = 0, vjust = 0) +
  theme_dft #here we specify our custom theme
```
...
Police force is made a factor, with each police force made a separate level.

To make a bar chart horizontal, the command **coord_flip** is included in the ggplot arguments.
...
accident_pf_small$Police_Force <- factor(accident_pf_small$Police_Force, levels = accident_pf_small$Police_Force)


# use the DfT colours and the DfT theme for the accidents by police force scatter chart
ggplot(data = accident_pf_small) +
  geom_col(mapping = aes(x = Police_Force, y = n, fill = Police_Force), col = "black") +
  coord_flip() +
  ...
  theme_dft +
  theme(legend.position = "none")#here we specify our custom theme
...
This requires using the **geom_bar** command in ggplot2 and setting the position to **stack**.

```{r}
# Make accident year a factor so year on x-axis is labelled individually (otherwise axis will be continuous)
road_acc$accident_year <- as.factor(road_acc$accident_year)
