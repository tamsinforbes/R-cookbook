...
The 'gg' in ggplot2 stands for 'grammar of graphics'. This is a way of thinking about plotting as having grammar elements 
that can be applied in succession to create a plot. This is the idea that you can build every graph from the same few components: a dataset, geoms (marks representing data points), a co-ordinate system and some other things.
...
You can adjust and tweak this theme and the colours to create plots in the correct style for your own publication. 
...
It can be used in two ways - either using ggplot2 (easier option) or using the plot_ly() wrapper directly which gives us more control.
...
Using plot_ly() is similar to ggplot2 in some ways as we build the plot in layers but we use the pipe operator rather than a plus sign, and ~ sign before the variables in our data:

```{r}
plot_ly(road_acc, x = ~accident_year, y = ~total) %>% # specify the data and x and y axes
  add_lines(color = ~name, colors = dft_colour[1:3]) %>% # colour lines by severity
  layout(title = "Accidents by severity, 2005 to 2017",
