...
library(base64enc) # for embedding html docs
library(mime) # for embedding html docs
library(xfun) # for embedding html docs
...
*Reading in Spatial Data*
```{r read in data, echo = TRUE, eval = FALSE, error = FALSE}
LAs <- sf::st_read("data/ONS Geography")
```
...
```{r 21, echo = TRUE, eval = FALSE, error = FALSE}
sf::st_geometry(LAs)
```
...
```{r 20, echo = TRUE, eval = FALSE, error = FALSE}
...
```{r 18, echo = TRUE, eval = FALSE, error = FALSE}
...
```{r 17, echo = FALSE, eval = FALSE, error = FALSE, warning = FALSE, message = FALSE}
LAs_pop <- read_csv("data/LAs_Populations.csv")

LAs_pop <- LAs_pop %>%
  dplyr::rename(lad19cd = Code, population = `All ages`, LA_name = Name)

LAs_pop <- LAs_pop %>%
  dplyr::select(lad19cd, population)
```
```{r 16, echo = TRUE, eval = FALSE, error = FALSE, message = FALSE}
summary(LAs_pop)
```
...
```{r 15, echo = TRUE, eval = FALSE, error = FALSE}
...
*Read in csv*
```{r 14, echo = FALSE, eval = FALSE, error = FALSE, warning = FALSE}

crash_data <- read.csv("data/road_traffic_accidents.csv")

crash_data <- crash_data %>% dplyr::select(-Longitude, -Latitude, -Local_Authority_.District., -Local_Authority_.Highway.)

crash_data$Date <- as.Date(crash_data$Date)


crash_data <- crash_data %>% dplyr::filter(Date >= as.Date("01/01/2018"))
```
...
```{r 13, echo = TRUE, eval = FALSE, error = FALSE, warning = FALSE}
...
```{r 12, echo = TRUE, eval = FALSE, error = FALSE, warning = FALSE}
...
LAs_casualties <- crashes_join %>% dplyr::group_by(lad19nm) %>% dplyr::summarise(casualties = sum(Number_of_Casualties)) # making an sf data frame called LAs_casualties which is the result of a simple group_by and summarise.

LAs_casualties %>% as.data.frame() %>% dplyr::arrange(desc(casualties)) %>% head()
```
...
```{r 11, echo = TRUE, eval = FALSE, error = FALSE, warning = FALSE}
LAs_casualties <- LAs_casualties %>% sf::st_set_geometry(NULL)

LAs <- dplyr::left_join(LAs, LAs_casualties, by = "lad19nm")
...
```
``` {r 6, echo = FALSE, eval = FALSE, error = FALSE, warning = TRUE}
xfun::embed_file("image/interactive1.html")
```
...
```
``` {r 4, echo = FALSE, eval = FALSE, error = FALSE, warning = TRUE}
xfun::embed_file("image/interactive2.html")
```
...
```
``` {r 2, echo = FALSE, eval = FALSE, error = FALSE, warning = TRUE}
xfun::embed_file("image/interactive3.html")
```
