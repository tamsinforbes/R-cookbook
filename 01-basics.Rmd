...
The base function `round()` in R rounds **half to even**, that is, half way is rounded to the nearest even number, for example the decimals below both round to the same value.
```{r}
round(3.5)
round(4.5)
```
This helps eliminate bias, more info on Wikipedia [round half to even](https://en.wikipedia.org/wiki/Rounding#Round_half_to_even)

If **round half to even** is not for you, you can use **round_half_up** from the {janitor} package, thus
```{r}
...
types; **factor** and **Date**. As well as some notes here this book also contains fuller sections on both
...
Date vectors are just double vectors with an additional class attribute set as "Date".  

**Note - The following borrows directly from Hadley's Advanced R site so we need to attribute appropriately and use correct licence**
...
The value of the double (which can be seen by stripping the class), represents the number of days since "1970-01-01"^[This special date is known as the Unix Epoch]:
